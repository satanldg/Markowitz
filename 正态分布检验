...
for p in range(4000):
    weights = np.random.random(4)
    weights /=np.sum(weights)
    port_returns.append(np.sum(returns.mean()*251*weights))
    port_variance.append(np.sqrt(np.dot(weights.T, np.dot(returns.cov()*251, weights))))
...
#黄星：标记最小方差组合
plt.plot(statistics(optv['x'])[1], statistics(optv['x'])[0], 'bv', markersize = 15.0)
plt.grid(True)
